<?php
/*
 Poll extension for MediaWiki
 Version: 1.5

 Authors:
 Sorin Sbarnea, 1.5 (2009)
 Based on an older version made by Eric David in 2006

 To activate the extension, include it from your LocalSettings.php with:

 include_once('${IP}/extensions/poll/poll.php');
 $wgPollShowVoters = false;
 ...
 Optional attributes for poll element:
 open  - will show users votes
 locked - no more votes are recorded
 small - use small css style
 ...
 To show list of polls where you didn't vote into
 <poll>
 REMAINING
 </poll>
...
function wfPoll() {
        ...
        // TODO: fix disableCache wich was disabled because was breaking: upgrade.php (mw 1.17)
  ...
  # Register CSS file for our select box:
        ...
        array(
                'rel'  => 'stylesheet',
                'type' => 'text/css',
                'href' => "$wgScriptPath/extensions/poll/poll.css"
         )
         );
...
function formatPoll( $input) {
    return "<div class=wp-poll>${input}</div>";
}
...
function renderPoll( $input, $argv=array() ) {
        global $wgParser,$wgUser,$wgScriptPath,$wgTitle,$wgRequest,$wgArticlePath,$wgPollShowVoters;
        ...
        $showvoters = $wgPollShowVoters;
        if(array_key_exists('open',$argv) && $argv['open']) $showvoters = true;


        if ($small) {$bginfo = ' '; $bginfo2 = ' '; }
        else $bginfo = $bginfo2 = ' ';
        if ($locked) {$bginfo = ' '; $bginfo2 = ' '; $small = '1'; }
        $wgParser->disableCache();
        $IP = wfGetIP();
        if ($wgUser->mName == "") $user = ""; else $user = $wgUser->mName;
  ...
  $str = "";
        ...
        if ($lines[1] == "STATS") // special param : stats
                {
                        ...
                        return formatPoll("There are $tab[1] polls and $tab[0] votes given by $tab[2] different people.".
                ...
                if ($lines[1] == "REMAINING") // special param : remaining polls
                {
                        ...
                        while ($polltxt = mysql_fetch_array($result))
                                { 
                                 ...
                                 if (sizeof($polltxt)>=1) {
                                    $tmp = split("\n",$polltxt[2]);
                                    $str .= /*'    '.*/$i.'.'.(++$j).'. <a href="' . str_replace('$1',$polltxt[1],str_replace(' ','%20',$wgArticlePath)).'#firstone">'.$polltxt[1].'</a> ('.$tmp[1].')<BR>';
                                    }
                                }
                                }
                        if ($str == "") $str = wfMsg('poll-nopolls'); // "There are no polls where you haven't voted yet.\n<BR>";
                        return formatPoll($str.$err);
                }
        ...
        if ($lines[1] == "CONTROL") // special param : remaining polls
                {
                        ...
                        return formatPoll("<form action=http://localhost/ip2dns.php method=post><input type=submit value='Calcul DNS'><input type=hidden name=txt value='".$b.
        ...
        if ($lines[1] == "CROSS") // special param : tableau croisé
                ...
                if (!empty($_POST['poll1'])) // the button has already been clicked
                {
                        ...
                        $str = "<table><tr><th rowspan=2>".
                                        "$split1[1]</td><th colspan=$nbansw2 >$split2[1]></th></tr><tr>";
                        for ($j=0;$j<$nbansw2;$j++)
                                $str .= "<td align=center>".$split2[$j+2]."</td>";
                        $str .= "</tr>";
                        for ($i=0;$i<$nbansw1;$i++)
                        {
                                $str .= "<tr><td align=right>".$split1[$i+2]."</td>";
                                for ($j=0;$j<$nbansw2;$j++)
                                {
                                ...
                                if ($norm == "line" && $tot_line[$i+2]>0) $str .= "<th>100%</th>";
                                if ($norm == "") $str .= "<th align=center>".round($tot_line[$i+2]*100/$total)."%</th>";
                                $str .= "</tr>";
                        ...
                        if ($norm == "row" || $norm == "")
                        {
                                ...
                                for ($j=0;$j<$nbansw2;$j++)
                                {
                                        ...
                                        $str .= "<th align=center>".($norm==""?round($tot_row[$j+2]*100/$total).'%':"100%")."</th>";
                                }
                                if ($norm == "") $str .= "<th>100%</th>";
                                $str .= "</tr>";
                        }
                        return formatPoll($str."</table>Votes: $total".'<BR>'.$comm.$err) ; /*.
                                ...
                                '<BR><form name=poll method=POST><input type=submit value="'.$lines[6].'">'.
                                        ...
                                        '<input type=hidden name=p2 value="'.$ID2.'"></form></div>');*/
                }
/*
...
//     $str .= "<p syle='color: red;'>[" . $wgUser->mId . "]</p>";

        if ($wgUser->mId == 0) // PROCESS VOTE ONLY FOR LOGGED USERS
      {
              $str .= "<p style='color: red;'>" . wfMsg('poll-notlogged') . "</p>";
      }
      else
    ...
    //    $str .= "KKK-KKK";
        ...
        // *******************************************************************************************
        $sql = "SELECT `poll_answer`, COUNT(*), GROUP_CONCAT(DISTINCT poll_user ORDER BY poll_user DESC SEPARATOR ';') AS voters
            "
                        ."\tFROM `poll_vote`\n"
                        ."\tWHERE "."`poll_id` = '".$ID."'"."\n"
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            if($showvoters) {
                ...
                $voters_a = split(';',$row[2]);
                foreach($voters_a as &$v) {
                   $url = str_replace( "$1", urlencode( "User:${v}" ), $wgArticlePath );
                   $v = "<a href='${url}'>${v}</a>";
                   }
                $voters[$row[0]] = implode($voters_a, ", " );
                }
             else {
               $voters[$row[0]] ='';
               }
...
and TIME_TO_SEC(title.poll_datemaxi)-TIME_TO_SEC(poll_info.poll_date)>2';
        ...
        for ($i=2;$i<$nbansw;$i++)
                        ...
                        '<input type="image" align=texttop src="'.$wgImagePath.'/button'.$small.'.gif" name="Vote" alt="Vote" name="Vote" '.
                                ($small==''?'width=23 height=20':'width=17 height=13').'> ').$lines[$i].'  '.$smalltxt2.'</form></td><td>&nbsp;&nbsp;</td>';
                        if ($total>0)
                                {
                                if (empty($poll_result[$i])) $res = 0; else $res = $poll_result[$i];
                                $str .= '<td class=wp-poll-width>';
                        ...
                        $str .= '<td class=wp-poll-voters>'.$voters[$i] .'</td></tr>';
                        }
        $str .= '</table><form name=poll method=POST action="#'.$ID.'">'.
                        '<input type="hidden" name="poll_ID" value="'.($ID).'">' ;
                        /*
        ...
        return formatPoll('<table><tr><td>'.$str.'</td></tr></table>');
 }
